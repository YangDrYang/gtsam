...
class XMLDocParser:
    ...
    def _get_class_xml_path(self, xml_folder: str, cpp_class: str) -> Path | None:
        ...
        class_index = index_root.find(f".//compound[@kind='class'][name='{cpp_class}']")
    ...
    def extract_docstring(
        ...
        if not xml_class_file:
            self.print_if_verbose(f"Could not find XML file for class {cpp_class}.")
        ...
        maybe_member_defs = self.get_member_defs_from_root(class_root, cpp_method)

        # Filter member definitions which don't match the given argument names
        member_defs, ignored_params = self.filter_member_defs(
            maybe_member_defs, method_args_names, cpp_class, cpp_method
        )  # Pass class/method for verbose printing
        ...
        if compound_def is None:
            self.print_if_verbose(f"Could not find <compounddef> in {xml_class_file}")
            return ""
        ...
        for base_ref in base_refs:
            ...
            parent_docstring = self.extract_docstring(
                xml_folder, parent_class_name, cpp_method, method_args_names
            )
    ...
    def get_member_defs_from_root(
        self, class_root: ET.Element, cpp_method: str
    ) -> list[ET.Element]:
    ...
    def filter_member_defs(
        self,
        maybe_member_defs: list[ET.Element],
        method_args_names: list[str],
        cpp_class: str,
        cpp_method: str,
    ):
        ...
        for maybe_member_def in maybe_member_defs:
            ...
            args_string_text = (
                args_string_elem.text
                if args_string_elem is not None
                else "[no argsstring]"
            )
            self.print_if_verbose(
            ...
            num_req_params = num_tot_params - sum(
                [1 if param.find("defval") is not None else 0 for param in params]
            )
            ...
            # parameters, required.
            if (
                len(method_args_names) != num_req_params
                and len(method_args_names) != num_tot_params
            ):
                self.print_if_verbose(f"    Parameter count mismatch. Skipping.")
                continue
            ...
            if (
                ...
                ignored_params = (
                    current_ignored  # Overwrite with ignored params from this match
                )
            else:
                self.print_if_verbose(
        ...
        return (
            member_defs,
            ignored_params,
        )  # Return the list associated with the last match found

    def determine_documenting_index(
        self,
        cpp_class: str,
        cpp_method: str,
    ...
    def get_formatted_docstring(self, member_def: "ET.Element", ignored_params: list):
        ...
        loc_info = (
            f"(from {location_elem.attrib.get('file', '?')}:{location_elem.attrib.get('line', '?')})"
        ...
        if detailed_description is not None:
            ...
            for element in list(detailed_description):
                ...
                if (
                    element.tag == "para"
                    and not element.findall(".//parameterlist")
                    and not element.findall(".//simplesect[@kind='return']")
            ...
            if has_detailed_para:
                ...
                has_content = True
                self.print_if_verbose(f"    Detailed Paras: {detailed_content}")

            # Add parameter docs
            parameter_list = detailed_description.find(
                ".//parameterlist[@kind='param']"
            )  # Ensure it's for 'param'
            param_docs = ""
            if parameter_list is not None:
                ...
                for i, parameter_item in enumerate(
                    ...
                    name_elem = parameter_item.find("./parameternamelist/parametername")
                    ...
                    desc = (
                        "".join(t for t in desc_elem.itertext() if t.strip()).strip()
            ...
            return_sect = detailed_description.find(".//simplesect[@kind='return']")
            return_doc = ""
            if return_sect is not None:
                # Find the paragraph text *within* the return section
                return_para = return_sect.find(
                    ".//para"
                )  # Use .// just in case it's nested further
                if return_para is not None:
                    return_text = "".join(
                        t for t in return_para.itertext() if t.strip()
                    ).strip()
                    if return_text:
...
if __name__ == "__main__":
    ...
    extracted_doc = parser.extract_docstring(
        xml_folder, cpp_class, cpp_method, method_args
    )
